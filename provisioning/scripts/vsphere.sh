...
copy_files()
{
   ...
   run scp ${SSH_OPTS} ${CWD}/conf/anchor.toml root@${TEST_HOST_IP2}:/etc/ha/anchor \
     ...
     clean_toml
        ...
        || fatal "Failed to copy keys to target host ${TEST_HOST_IP}"
        ...
        run ssh ${SSH_OPTS} root@${TEST_HOST_IP} 'sudo chmod 600 /root/.ssh/id_rsa' || fatal "Failed to execute command on the target host ${TEST_HOST_IP}"
